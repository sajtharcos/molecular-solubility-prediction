...
# 2. Define the list of column names
cols = df.columns.tolist()

features_list = []
target_list = []

# 1. Create the fingerprint generator once before the loop
from rdkit.Chem import rdFingerprintGenerator
gen = rdFingerprintGenerator.GetMorganGenerator(radius=2, fpSize=2048)

# 2. Use the generator inside the loop
for index, row in df.iterrows():
    mol = Chem.MolFromSmiles(row["smiles"])
    if mol:
        # New method: directly generate bit vector
...
optimizer = torch.optim.Adam(model.parameters(), lr=0.001)  # Optimizer ("teacher")
